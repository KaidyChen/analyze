-module (analyze_central_ac_used_time_all_night_of_month).
...
do_work(Meter_type, Meter, Year, Month) ->
    Every_day_all_night_used_time_of_day_filepath = analyze_util:get_every_day_all_night_used_time_filepath(Year, Month, Meter),
    case get_all_night_used_time_line_list(Every_day_all_night_used_time_of_day_filepath) of
        {ok, []} -> ok;
        {ok, Data_line_list} ->
            Fun = fun
                (Data_line) ->
            ...
            Used_time_all_night_of_month_filepath = analyze_util:get_used_time_all_night_of_month_filepath(Year, Meter),
            case append_to_file(Used_time_all_night_of_month_filepath, New_content) of
                ok -> ok;
                {error, Reason} ->
            ...
            ok;
        {error, Reason} ->
            ?ERROR("get_all_night_used_time_line_list(~p) is error: ~p", [Every_day_all_night_used_time_of_day_filepath, Reason])
    end.

get_all_night_used_time_line_list(Every_day_all_night_used_time_of_day_filepath) ->
    case file:read_file(Every_day_all_night_used_time_of_day_filepath) of
        {ok, Data_bin} ->
