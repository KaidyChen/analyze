...
handle_info({report_come, Msg_type = ?DATAMSG, Meter_type, Meter, Meter_blob}, State) ->
    ...
    case Meter_type of
        ?LIGHTING_TYPE ->
            LightingStatus = list_to_integer(analyze_meter_blob_util:get_relay_status_str(Meter_blob)),
            %% 0 表示灯灭，1 表示灯亮, 灯的不是表示继电器状态
            analyze_meter_on_off_status:insert(Meter_type, Meter, LightingStatus);
        _ ->
            ok
    end,            
...
handle_info({timeout, TimerRef, ?REPORT_TIMEOUT_MSG}, State)  ->
    ...
    case lists:member(Miss_count, ?ALRAM_MISS_NUMBER_LIST) of
        true ->
            ...
            ?PRINT("~p/~p Alram: miss data ~p~n", [Meter_type, Meter, Miss_count]);
        false -> ok
    end,
...
report_come_work(State, Msg_type, Meter_blob) ->
    ...
    case app_util:env_proplists(patch_hour_data, enable) of
        true ->
            case Meter_time of
                undefined -> ok;
