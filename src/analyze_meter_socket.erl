...
init([Meter_field]) ->
    ...
    %% 卡座活动流
    Workflow = analyze_meter_util:workflow_init(Meter_type, Meter, Slave_label),
...
handle_info({report_come, Msg_type = ?DATAMSG, Meter_type, Meter, Meter_blob}, State) ->
    ...
    %%------------------------------------------------------------------------
    Slave_label = analyze_meter_field:get_slave_label_by_meter_field(Meter_field),
    New_workflow = analyze_meter_util:update_workflow(Slave_label, Meter_type, Meter, Workflow, Meter_blob),
    %% 更新存储实时活动流
    analyze_realtime_workflow:insert(Meter_type, Meter, New_workflow),
    %%--------------------------------------------------------------------------
    %% end
    %%--------------------------------------------------------------------------
        ...
        {error, _} ->
            case analyze_meter_util:get_base_quantity(Meter_type, Meter, Last_report_time, Now, Cq_weight) of
                {ok, Electric_power} ->
                    ...
                    analyze_meter_quota_server:update_quota(New_meter_quota);
                {error, Reason} ->
                    ?ERROR("~p:~p ~p/~p can't get the last electric_power:~p~n", [Meter_type, Meter, Reason])
            end
    end,
    {noreply, State};
...
handle_info({hour_data_result, {ok, Result_str_tmp}}, State) ->
   #state {
    ...
    ?PRINT("Hour_data_last_datetime:~p~n", [Hour_data_last_datetime]),
    ?PRINT("Result_str_tmp:~p~n", [Result_str_tmp]),
    New_hour_data_last_datetime = case analyze_meter_hour_data:parse_result(Result_str_tmp) of
        {ok, Datetime, Electric_power} ->
            ?PRINT("Datetime:~p Electric_power:~p~n", [Datetime, Electric_power]),
        ...
        {error, Reason} ->
            ?PRINT("parse_result error: ~p~n", [Reason]),
            Hour_data_last_datetime
    end,
